...
pub struct GrokEngine(AcpEngine);

impl Default for GrokEngine {
    fn default() -> Self {
        let agent = crate::acp::find_known_acp_agent("acp-grok")
            .expect("acp-grok is a row in acp::KNOWN_ACP_AGENTS");
        GrokEngine(AcpEngine::new(agent))
    }
}

impl HeadlessEngine for GrokEngine {
    fn support(&self) -> super::EngineSupport {
        crate::headless::shared::engine_support(proto::AgentKind::Grok)
    }
