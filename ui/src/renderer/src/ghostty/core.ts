...
import { GhosttyRuntime, loadGhosttyRuntime, type TypeField as TypeFieldRef } from "./runtime";
...
export interface GhosttyRow {
  ...
  /** Memoised by `ghosttyRowText` on first read; left `null` here rather than a
   * self-replacing getter, which used to push V8 into dictionary mode on every dirty row. */
  text?: string | null;
...
// Interned by packed RGB: every cell read mints fresh colors and the key space is
// only 24-bit, so unbounded caching is cheap and gives ghosttyColorsEqual an identity
// fast path that the run-boundary checks in renderer.ts hit on nearly every cell.
const colorCache = new Map<number, GhosttyColor>();

function ghosttyColor(r: number, g: number, b: number): GhosttyColor {
  const key = (r << 16) | (g << 8) | b;
  const hit = colorCache.get(key);
  if (hit) return hit;
  const color = Object.freeze({ r, g, b });
...
export class GhosttyTerminalCore {
  ...
  static async create(
    ...
    const core = new GhosttyTerminalCore(runtime ?? (await loadGhosttyRuntime()));
    try {
      core.initialize(cols, rows, cellWidth, cellHeight, theme, onPtyData, options);
      return core;
    } catch (error) {
      core.dispose();
      throw error;
  ...
  write(data: string | Uint8Array): void {
    ...
    const pointer = this.writeBuffer(bytes.length);
    this.runtime.copyInto(pointer, bytes);
    this.runtime.call("ghostty_terminal_vt_write", this.terminal, pointer, bytes.length);
  }

  private writeScratch = 0;
  private writeScratchSize = 0;

  private writeBuffer(size: number): number {
    if (size <= this.writeScratchSize) return this.writeScratch;
    if (this.writeScratch !== 0) this.runtime.free(this.writeScratch, this.writeScratchSize);
    const next = Math.max(4096, 1 << (32 - Math.clz32(size - 1)));
    this.writeScratch = this.runtime.allocUninit(next);
    this.writeScratchSize = next;
    return this.writeScratch;
  }

  private graphemeScratch = 0;
  private graphemeScratchSize = 0;

  private graphemeBuffer(size: number): number {
    if (size <= this.graphemeScratchSize) return this.graphemeScratch;
    if (this.graphemeScratch !== 0) {
      this.runtime.free(this.graphemeScratch, this.graphemeScratchSize);
    }
    const next = Math.max(64, 1 << (32 - Math.clz32(size - 1)));
    this.graphemeScratch = this.runtime.allocUninit(next);
    this.graphemeScratchSize = next;
    return this.graphemeScratch;
  }
  ...
  importSnapshot(state: Uint8Array): boolean {
    ...
    const pointer = this.runtime.allocUninit(state.length);
    try {
      this.runtime.copyInto(pointer, state);
      const result = this.runtime.call(
        "houston_vt_snapshot_import",
        this.terminal,
        pointer,
        state.length,
      );
      ...
      this.runtime.free(pointer, state.length);
  ...
  exportSnapshot(historyRows: number): Uint8Array | null {
    this.ensureActive();
    const outPointer = this.runtime.allocOpaque();
    const outLength = this.runtime.allocOpaque();
    try {
      const result = this.runtime.call(
      ...
      const pointer = this.runtime.readPointer(outPointer);
      const length = this.runtime.readPointer(outLength);
      if (pointer === 0) return null;
      const bytes = this.runtime.u8().slice(pointer, pointer + length);
      this.runtime.call("ghostty_free", 0, pointer, length);
      return bytes;
    } finally {
      this.runtime.freeOpaque(outPointer);
      this.runtime.freeOpaque(outLength);
    }
  }
  ...
  // Option 23 is the embedder's default cursor-blink state -- the state a session
  // starts in and returns to on a DECSCUSR reset -- so a program that requests a
  // specific style via DECSCUSR or DEC mode 12 still wins over this.
  private applyDefaultCursorBlink(): void {
    const blink = this.runtime.alloc(1);
    this.runtime.bytes(blink, 1)[0] = this.defaultCursorBlink ? 1 : 0;
  ...
  setDefaultCursorBlink(enabled: boolean): void {
    this.ensureActive();
    this.defaultCursorBlink = enabled;
    this.applyDefaultCursorBlink();
  }
  ...
  gridSize(): { cols: number; rows: number } {
    ...
    this.assertSuccess(
      ...
      this.runtime.renderStateUpdate(this.renderState, this.terminal),
    );
    return { cols: this.getU16(RENDER_DATA.cols), rows: this.getU16(RENDER_DATA.rows) };
  }
  ...
  snapshot(): GhosttySnapshot {
    ...
    const foreground = this.getColor(RENDER_DATA.foreground, ghosttyColor(229, 231, 235));
    const background = this.getColor(RENDER_DATA.background, ghosttyColor(0, 0, 0));
    ...
    if (dirty !== 0) {
      ...
      this.runtime.renderStateSet(this.renderState, 0, this.scratch);
    }
  ...
  dispose(): void {
    ...
    if (this.writeScratch) this.runtime.free(this.writeScratch, this.writeScratchSize);
    if (this.graphemeScratch) this.runtime.free(this.graphemeScratch, this.graphemeScratchSize);
    this.writeScratch = 0;
    this.writeScratchSize = 0;
    this.graphemeScratch = 0;
    this.graphemeScratchSize = 0;
    for (const slot of [
      this.mouseEventSlot,
  ...
  private readRow(
    ...
    this.assertSuccess(
      ...
      this.runtime.renderStateRowGet(iterator, ROW_DATA.raw, this.scratch),
    );
    ...
    this.assertSuccess(
      ...
      this.runtime.rowGet(rawRow, 2, this.scratch + 8),
    );
    const isWrapContinuation = this.runtime.bytes(this.scratch + 8, 1)[0] !== 0;
    this.runtime.bytes(this.scratch + 8, 1)[0] = 0;
    this.assertSuccess(
      "ghostty_row_get(wrap)",
      this.runtime.rowGet(rawRow, 1, this.scratch + 8),
    );
    ...
    while (cells.length < cols && this.runtime.renderStateRowCellsNext(cellsIterator) !== 0) {
      ...
      this.runtime.setFieldAt(this.style, sf.sizeField, styleSize);
      this.runtime.renderStateRowCellsGet(cellsIterator, CELL_DATA.style, this.style);
      const inverse = this.runtime.readFieldAt(this.style, sf.inverse) !== 0;
      if (inverse) [foreground, background] = [background, foreground];
      if (this.runtime.readFieldAt(this.style, sf.faint) !== 0) {
        foreground = blend(foreground, background);
      }
      const graphemeLength = this.getCellU32(cellsIterator, CELL_DATA.graphemesLength);
      let text = "";
      if (graphemeLength > 0) {
        const codepoints = this.graphemeBuffer(graphemeLength * 4);
        if (
          this.runtime.renderStateRowCellsGet(
            cellsIterator,
            CELL_DATA.graphemes,
            codepoints,
          ) === GHOSTTY_SUCCESS
        ) {
          const codepointView = this.runtime.dv();
          if (graphemeLength === 1) {
      ...
      if (text.length === 0 && cells.at(-1)?.text.length) {
        ...
        this.assertSuccess(
          ...
          this.runtime.cellGet(rawCell, RAW_CELL_DATA.wide, this.scratch + 8),
        );
  ...
  private styleFieldCache: {
    size: number;
    sizeField: TypeFieldRef;
    inverse: TypeFieldRef;
    faint: TypeFieldRef;
    bold: TypeFieldRef;
    italic: TypeFieldRef;
    invisible: TypeFieldRef;
    strikethrough: TypeFieldRef;
    overline: TypeFieldRef;
    underline: TypeFieldRef;
  } | null = null;

  private styleFields(): NonNullable<GhosttyTerminalCore["styleFieldCache"]> {
    if (this.styleFieldCache) return this.styleFieldCache;
    const f = (name: string): TypeFieldRef => this.runtime.field("GhosttyStyle", name);
    this.styleFieldCache = {
      size: this.runtime.layout("GhosttyStyle").size,
      sizeField: f("size"),
      inverse: f("inverse"),
      faint: f("faint"),
      bold: f("bold"),
  ...
  private getU16(data: number): number {
    this.runtime.zero(this.scratch, 2);
    this.assertSuccess(
      "ghostty_render_state_get",
      this.runtime.renderStateGet(this.renderState, data, this.scratch),
    );
    return this.runtime.dv().getUint16(this.scratch, true);
  }

  private getU32(data: number): number {
    this.runtime.zero(this.scratch, 4);
    this.assertSuccess(
      "ghostty_render_state_get",
      this.runtime.renderStateGet(this.renderState, data, this.scratch),
    );
    return this.runtime.dv().getUint32(this.scratch, true);
  }

  private getBool(data: number): boolean {
    this.runtime.u8()[this.scratch] = 0;
    this.assertSuccess(
      "ghostty_render_state_get",
      this.runtime.renderStateGet(this.renderState, data, this.scratch),
    );
    return this.runtime.u8()[this.scratch] !== 0;
  }

  private getColor(data: number, fallback: GhosttyColor): GhosttyColor {
    this.runtime.zero(this.scratch, 3);
    const result = this.runtime.renderStateGet(this.renderState, data, this.scratch);
  ...
  private getRowBool(iterator: number, data: number): boolean {
    this.runtime.u8()[this.scratch] = 0;
    return (
      this.runtime.renderStateRowGet(iterator, data, this.scratch) === GHOSTTY_SUCCESS &&
      this.runtime.u8()[this.scratch] !== 0
    );
  }
  ...
  private getCellU32(iterator: number, data: number): number {
    this.runtime.zero(this.scratch, 4);
    const result = this.runtime.renderStateRowCellsGet(iterator, data, this.scratch);
    return result === GHOSTTY_SUCCESS ? this.runtime.dv().getUint32(this.scratch, true) : 0;
  }

  private getCellBool(iterator: number, data: number): boolean {
    this.runtime.u8()[this.scratch] = 0;
    return (
      this.runtime.renderStateRowCellsGet(iterator, data, this.scratch) === GHOSTTY_SUCCESS &&
      this.runtime.u8()[this.scratch] !== 0
    );
  }
  ...
  private readColor(pointer: number): GhosttyColor {
    const bytes = this.runtime.u8();
    return ghosttyColor(bytes[pointer] ?? 0, bytes[pointer + 1] ?? 0, bytes[pointer + 2] ?? 0);
  }
...
export function ghosttyRowText(row: GhosttyRow): string {
  const cached = row.text;
