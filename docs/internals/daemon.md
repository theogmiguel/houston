...
| headless | `headless/` | The `HeadlessEngine` seam every headless caller drives: `claude.rs`, `codex.rs`, `acp.rs` (with `opencode.rs`/`grok.rs` as rows over it); `shared.rs` holds the adapter plumbing and the engine-support table every engine reads |
...
Everything that runs an agent with no PTY — a routine run — goes through one seam,
...
- **Codex** is `codex exec --json` with the prompt on argv. `exec` never prompts, so the
  ...
  ask, which is not representable) and `danger-full-access` for bypass. `exec resume <id>`
  takes neither cwd nor sandbox. The purpose briefing
...
- **OpenCode and Grok** speak ACP. The engine writes `initialize` and `session/new` (or
  ...
  `session/request_permission` itself in accept-edits/bypass mode; in ask mode the
